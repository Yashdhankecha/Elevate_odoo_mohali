            ...
            <div className="max-w-xl w-full relative z-10">
                <div className="glass-card rounded-2xl p-8 md:p-12 animate-slide-up">
                    <div className="text-center mb-10">
                        <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-slate-100 relative">
                            <FaClock className="h-10 w-10 text-slate-400 animate-spin-slow" />
                            <div className="absolute -top-1 -right-1">
                                <ShieldCheck className="h-8 w-8 text-slate-900 fill-slate-50" />
                            </div>
                        </div>
                        ...
                        <p className="text-slate-500 font-medium font-sans">Our admins are verifying your <span className="text-slate-900 font-bold uppercase text-xs tracking-widest">{userRole}</span> profile.</p>
                    </div>

                    <div className="bg-white/40 rounded-xl p-6 mb-8 border border-white/60 backdrop-blur-md shadow-sm">
                        <h3 className="flex items-center gap-2 text-slate-900 font-bold mb-5 text-sm">
                            <Sparkles className="h-4 w-4 text-slate-900" /> Unlock after approval:
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {features.map((f, i) => (
                                <div key={i} className="flex items-center gap-3 bg-white/50 p-3 rounded-lg border border-white/50">
