        ...
        <div className="relative z-10 space-y-8 max-w-4xl">
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-3 px-8 py-5 rounded-[2rem] font-black uppercase tracking-widest text-xs transition-all duration-300 ${showFilters ? 'bg-white text-blue-600' : 'bg-white/20 backdrop-blur-md text-white hover:bg-white/30'
                }`}
            >
              <FaFilter size={14} />
          ...
          <div key={job.id} className="glass-card p-10 rounded-[3rem] border-white/50 bg-white/70 hover:shadow-2xl hover:shadow-blue-200/40 transition-all duration-500 hover-lift group">
            <div className="flex justify-between items-start mb-8">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {job.isOnCampus && (
                      <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[9px] font-black uppercase tracking-widest rounded-md border border-indigo-200">
                        On-Campus Drive
                      </span>
                    )}
                  </div>
              ...
              <button
                ...
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${favorites.includes(job.id) ? 'bg-rose-50 text-rose-500 shadow-inner' : 'bg-gray-50 text-gray-300 hover:text-rose-400'
                  }`}
              >
                <FaHeart />
              </button>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              {job.isOnCampus && job.eligibility && (
                <div className="text-right">
                  {job.eligibility.eligible ? (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-100">
                      <FaCheckCircle size={10} /> Eligible to Apply
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-rose-50 text-rose-500 text-[10px] font-black uppercase tracking-widest rounded-full border border-rose-100" title={job.eligibility.issues?.join(' • ')}>
                      <FaTimes size={10} /> Not Eligible
                    </span>
                  )}
                </div>
              )}
          ...
          {mode === 'view' ? (
            <div className="grid lg:grid-cols-12 gap-10">
              ...
              <div className="lg:col-span-4 space-y-8">
                <div className="glass-card p-8 rounded-[2.5rem] border-white/50 bg-gray-50/50">
                  ...
                  <div className="space-y-6">
                    {[
                      ...
                      job.isOnCampus && job.tentativeDriveDate && { label: 'Drive Date', val: new Date(job.tentativeDriveDate).toLocaleDateString('en-IN') }
                    ].filter(Boolean).map((stat, i) => (
                      <div key={i} className="flex justify-between items-end border-b border-gray-200/50 pb-4">
                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{stat.label}</p>
                        <span className="text-xs font-black text-gray-800 uppercase text-right leading-tight max-w-[60%]">{stat.val}</span>
                      </div>
                    ))}
                  </div>
                </div>

                {job.isOnCampus && job.pptRequired && job.pptDetails && (
                  <div className="glass-card p-8 rounded-[2rem] border-white/50 relative overflow-hidden bg-gradient-to-br from-indigo-50 to-blue-50">
                    <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-4 flex items-center gap-2"><FaBuilding /> Pre-Placement Talk</h4>
                    <div className="space-y-3">
                      {job.pptDetails.dateTime && (
                        <div className="flex justify-between text-[11px] font-bold text-indigo-900 border-b border-indigo-100/50 pb-2">
                          <span className="opacity-70">Schedule</span>
                          <span>{new Date(job.pptDetails.dateTime).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                        </div>
                      )}
                      {job.pptDetails.venue && (
                        <div className="flex justify-between text-[11px] font-bold text-indigo-900 border-b border-indigo-100/50 pb-2">
                          <span className="opacity-70">Venue</span>
                          <span className="text-right max-w-[60%]">{job.pptDetails.venue}</span>
                        </div>
                      )}
                      {job.pptDetails.duration && (
                        <div className="flex justify-between text-[11px] font-bold text-indigo-900">
                          <span className="opacity-70">Duration</span>
                          <span>{job.pptDetails.duration}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {job.isOnCampus && job.eligibility && job.eligibility.eligible === false && (
                  <div className="glass-card p-6 rounded-[2rem] border-rose-100 bg-rose-50/80">
                    <h4 className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-3 flex items-center gap-2"><FaTimes /> Eligibility Issues</h4>
                    <ul className="space-y-2">
                      {job.eligibility.issues.map((issue, i) => (
                        <li key={i} className="text-[11px] font-bold text-rose-700 leading-snug">• {issue}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        {mode === 'view' && (
          <div className="px-10 py-8 bg-gray-50/50 border-t border-gray-100/50 flex gap-4">
            <button
              ...
              disabled={job.isOnCampus && job.eligibility && job.eligibility.eligible === false}
              className={`flex-1 py-5 rounded-[1.8rem] font-black uppercase tracking-widest text-[10px] shadow-2xl transition-all flex items-center justify-center gap-3 group ${job.isOnCampus && job.eligibility && job.eligibility.eligible === false
                  ? 'bg-gray-200 text-gray-400 shadow-none cursor-not-allowed'
                  : 'bg-blue-600 text-white shadow-blue-200 hover:bg-blue-700'
                }`}
            >
              {job.isOnCampus && job.eligibility && job.eligibility.eligible === false ? (
                <span className="flex items-center gap-2"><FaTimes /> INELIGIBLE FOR DRIVE</span>
              ) : (
                <>
